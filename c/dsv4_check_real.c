...
#undef RVEC
             ...
             * ------------------------------------------------------------- */
            {
                ...
                int pre = 200, n = 128;         /* pre crosses the window AND a block */
                if (pre + n > fmax) { n = fmax > 264 ? fmax - 200 : 64;
                                      pre = fmax > 264 ? 200 : fmax - n - 1; }
                if (pre < 1 || n < 1) {
                ...
                dsv4_attention_prefill(&c, &w, xs + (size_t)pre * c.dim, tks, 1, n,
                                       nt, got);
